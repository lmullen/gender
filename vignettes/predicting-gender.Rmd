...
These datasets are available in the [genderdata package](https://github.com/ropensci/genderdata). This package is not available on CRAN, but it can be installed from the [rOpenSci repository](http://packages.ropensci.org/): `install.packages("genderdata", repos = "http://packages.ropensci.org")`. If you use the SSA or IPUMS method without the data package installed, you will be prompted to install it. This vignetee uses the `demo` method, which uses only the one hundred most popular names from the SSA dataset.
