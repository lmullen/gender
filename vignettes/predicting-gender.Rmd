...
These datasets are available in the [genderdata package](https://github.com/ropensci/gender-data-pkg). This package is not available on CRAN, but it can be installed using [devtools](http://cran.r-project.org/web/packages/devtools/index.html): `devtools::install_github("ropensci/gender-data-pkg")`. If you use the SSA or IPUMS method without the data package installed, you will be prompted to install it. This vignetee uses the `demo` method, which uses only the one hundred most popular names from the SSA dataset.
...
sample_years_ipums  <- c(rep(c(1830, 1900), 2))
...
If we want to use the same range of years for all of the names, we can pass the names vector to the `gender()` function and use a constant range of years (in this case, the minimum and maximum year in the data set). The result is a list of lists. While we could deal with that data structure if we needed to, it is much easier to convert the list of lists to a data frame:
...
The data sets which are used in the `gender` package are available through the `genderdata` package. You can access these datasets for yourself by loading that data package.

```{r eval=FALSE}
library(genderdata)
```

Then you can see the names of the data sets in the package:
...
For documentation and citations to each of the data sets, search the help: for example, `?ssa_national`.

You may find the related data in Hadley Wickham's [babynames](https://github.com/hadley/babynames) package, for example the `lifetables` data, useful for interpreting the SSA data.
